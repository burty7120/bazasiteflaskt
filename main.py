...
class Sneaker(db.Model):
    ...
    image = db.Column(db.LargeBinary, nullable=False)  # Store image as BYTEA
...
def init_db():
    ...
    try:
        ...
        if Sneaker.query.count() == 0:
            # Placeholder images as base64 (replace with actual base64 strings or load files locally for testing)
            sample_sneakers = [
                Sneaker(
                    name="Nike Air Max 270",
                    image=base64.b64decode("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg=="),  # Placeholder
                    sizes="40,41,42,43,44",
                    created_at=datetime.utcnow()
                ),
                Sneaker(
                    name="Adidas Yeezy Boost",
                    image=base64.b64decode("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M/wHwAEBgIApD5f8QAAAABJRU5ErkJggg=="),  # Placeholder
                    sizes="41,42,43,44,45,46",
                    created_at=datetime.utcnow()
                ),
                Sneaker(
                    name="Puma RS-X",
                    image=base64.b64decode("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYGBgAAAABQABEgB8pQAAAABJRU5ErkJggg=="),  # Placeholder
...
def get_sneakers():
    try:
        ...
        return jsonify([{
            'id': sneaker.id,
            'name': sneaker.name,
            'image': base64.b64encode(sneaker.image).decode('utf-8'),  # Return image as base64
            'sizes': sneaker.sizes
    ...
    if request.method == 'POST':
        try:
            ...
            if file:
                ...
                sneaker = Sneaker(name=name, image=image_data, sizes=sizes)
